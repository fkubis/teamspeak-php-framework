...
class Channel extends AbstractNode
{
    ...
    public function message($msg, $cpw = null)
    {
        ...
        $this->execute(
            "sendtextmessage",
            array("msg" => $msg, "target" => $this->getId(), "targetmode" => TeamSpeak3::TEXTMSG_CHANNEL)
        );
    }
    ...
    public function sendPluginCmd($plugin, $data, $cpw = null, $subscribed = false)
    {
        ...
        $this->execute(
            ...
            array(
                "name" => $plugin,
                "data" => $data,
                "targetmode" => $subscribed ? TeamSpeak3::PLUGINCMD_CHANNEL_SUBSCRIBED : TeamSpeak3::PLUGINCMD_CHANNEL
            )
        );
    }
