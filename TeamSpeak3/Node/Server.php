...
class Server extends AbstractNode
{
    ...
    {
        return array_keys(
            $this->execute("clientdbfind", array("pattern" => $pattern, ($uid) ? "-uid" : null))->toAssocArray("cldbid")
        );
    }
    ...
    public function clientPermList($cldbid, $permsid = false)
    {
        ...
        return $this->execute("clientpermlist", array("cldbid" => $cldbid, $permsid ? "-permsid" : null))->toAssocArray(
            $permsid ? "permsid" : "permid"
        );
    }
    ...
    public function serverGroupIdentify($mode = TeamSpeak3::GROUP_IDENTIFIY_STRONGEST)
    {
        ...
        $best_guess_profile = ($mode == TeamSpeak3::GROUP_IDENTIFIY_STRONGEST) ? array_shift($profiles) : array_pop(
            $profiles
        );
    ...
    public function message($msg)
    {
        $this->execute(
            "sendtextmessage",
            array("msg" => $msg, "target" => $this->getId(), "targetmode" => TeamSpeak3::TEXTMSG_SERVER)
        );
    }
    ...
    public function sendPluginCmd($plugin, $data)
    {
        $this->execute(
            "plugincmd",
            array("name" => $plugin, "data" => $data, "targetmode" => TeamSpeak3::PLUGINCMD_SERVER)
        );
    }
